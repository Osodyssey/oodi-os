...
const USERS = {
  alice: { password: process.env.ALICE_PW || 'password', allowedSecrets: ['ai'] },
...
function authMiddleware(req,res,next){
  ...
  try{
    ...
    next();
  }catch(e){return res.status(401).json({ error: 'invalid auth' })}
}
...
app.post('/api/get-secret-token', authMiddleware, (req,res)=>{
  ...
  const token = jwt.sign({ name, scope: ['generate'], iss: 'oodi-server', aud: 'oodi-proxy', jti }, SECRET, { expiresIn: '60s' });
...
function verifyToken(req,res,next){
  ...
  try{
    ...
    if(!payload.scope || !payload.scope.includes('generate')) return res.status(403).json({ error: 'insufficient scope' });
...
app.post('/api/proxy/ai', verifyToken, async (req,res)=>{
  try{
    ...
    // Call external AI - here we simulate by calling internal endpoint
    const response = await fetch('http://localhost:' + (process.env.PORT || 3000) + '/external/ai/generate', {
      method: 'POST',
...
app.post('/external/ai/generate', (req,res)=>{
  ...
  if(!key || key !== API_KEYS.ai) return res.status(403).json({ error: 'invalid external api key' });
...
const TODOS = [];
app.post('/api/log', (req,res)=>{ console.log('CLIENT LOG:', req.body.msg); res.json({ ok:true }); });
app.post('/api/todo/add', (req,res)=>{ TODOS.push(req.body.text); res.json({ ok:true }); });
app.post('/api/todo/list', (req,res)=>{ res.json({ ok:true, items: TODOS }); });

const PORT = process.env.PORT || 3000;
app.listen(PORT, ()=> console.log('Secure Oodi server running on', PORT));
